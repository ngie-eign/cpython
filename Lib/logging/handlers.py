...
class NTEventLogHandler(logging.Handler):
    ...
    def __init__(self, appname, dllname=None, logtype="Application"):
        ...
        try:
            ...
            print(
                "The Python Win32 extensions for NT (service, event logging) "
                "are not available."
            )
            self._welu = None
    ...
    def emit(self, record):
        ...
        if self._welu:
            try:
                message_id = self.getMessageID(record)
                event_cat = self.getEventCategory(record)
                event_type = self.getEventType(record)
                msg = self.format(record)
                self._welu.ReportEvent(
                    self.appname, message_id, event_cat, event_type, [msg]
                )
            except Exception:
                self.handleError(record)
